...
M.list = function()
  ...
  return fzf.files({
    ...
    actions = {
      ["default"] = function(selected) M.load({ name = selected[1] }) end,
      ["ctrl-x"] = { function(selected) M.delete({ name = selected[1] }) end, fzf.actions.resume },
    },
  })
end
...
M.update = function(opts)
  if not opts then opts = {} end
  if opts.ask_confirmation == nil then opts.ask_confirmation = true end
